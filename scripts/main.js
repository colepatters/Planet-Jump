...
self["c3_base64files"];this._localFileStrings={};await this._ConvertDataUrisToBlobs();for(let i=0,len=opts.engineScripts.length;i<len;++i){const src=opts.engineScripts[i].toLowerCase();if(this._localFileStrings.hasOwnProperty(src))opts.engineScripts[i]={isStringSrc:true,str:this._localFileStrings[src]};else if(this._localFileBlobs.hasOwnProperty(src))opts.engineScripts[i]=URL.createObjectURL(this._localFileBlobs[src])}}if(opts.baseUrl)this._baseUrl=opts.baseUrl;else{const origin=location.origin;this._baseUrl=
(origin==="null"?"file:///":origin)+location.pathname;const i=this._baseUrl.lastIndexOf("/");if(i!==-1)this._baseUrl=this._baseUrl.substr(0,i+1)}if(opts.workerScripts)this._workerScriptURLs=opts.workerScripts;const messageChannel=new MessageChannel;this._messageChannelPort=messageChannel.port1;this._messageChannelPort.onmessage=e=>this["_OnMessageFromRuntime"](e.data);if(window["c3_addPortMessageHandler"])window["c3_addPortMessageHandler"](e=>this._OnMessageFromDebugger(e));this._jobScheduler=new self.JobSchedulerDOM(this);
await this._jobScheduler.Init();if(typeof window["StatusBar"]==="object")window["StatusBar"]["hide"]();if(typeof window["AndroidFullScreen"]==="object")window["AndroidFullScreen"]["immersiveMode"]();if(this._useWorker)await this._InitWorker(opts,messageChannel.port2);else await this._InitDOM(opts,messageChannel.port2)}_GetWorkerURL(url){let ret;if(this._workerScriptURLs.hasOwnProperty(url))ret=this._workerScriptURLs[url];else if(url.endsWith("/workermain.js")&&this._workerScriptURLs.hasOwnProperty("workermain.js"))ret=
this._workerScriptURLs["workermain.js"];else if(this._exportType==="playable-ad"&&this._localFileBlobs.hasOwnProperty(url.toLowerCase()))ret=this._localFileBlobs[url.toLowerCase()];else ret=url;if(ret instanceof Blob)ret=URL.createObjectURL(ret);return ret}async CreateWorker(url,baseUrl,workerOpts){if(url.startsWith("blob:"))return new Worker(url,workerOpts);if(this.IsiOSCordova()&&location.protocol==="file:"){const arrayBuffer=await this.CordovaFetchLocalFileAsArrayBuffer(this._scriptFolder+url);
...
"undefined"}}async _InitWorker(opts,port2){const workerMainUrl=this._GetWorkerURL(opts.workerMainUrl);this._worker=await this.CreateWorker(workerMainUrl,this._baseUrl,{type:"module",name:"Runtime"});this._canvas=document.createElement("canvas");this._canvas.style.display="none";const offscreenCanvas=this._canvas["transferControlToOffscreen"]();document.body.appendChild(this._canvas);window["c3canvas"]=this._canvas;this._worker.postMessage(Object.assign(this._GetCommonRuntimeOptions(opts),{"type":"init-runtime",
"isInWorker":true,"messagePort":port2,"canvas":offscreenCanvas,"workerDependencyScripts":opts.workerDependencyScripts||[],"engineScripts":opts.engineScripts,"projectScripts":opts.projectScripts,"mainProjectScript":opts.mainProjectScript,"projectScriptsStatus":self["C3_ProjectScriptsStatus"]}),[port2,offscreenCanvas,...this._jobScheduler.GetPortTransferables()]);this._domHandlers=domHandlerClasses.map(C=>new C(this));this._FindRuntimeDOMHandler();self["c3_callFunction"]=(name,params)=>this._runtimeDomHandler._InvokeFunctionFromJS(name,
...
()=>{this._PostRuntimeEvent("window-blur",{"parentHasFocus":ParentHasFocus()});this._mousePointerLastButtons=0});window.addEventListener("keydown",e=>this._OnKeyEvent("keydown",e));window.addEventListener("keyup",e=>this._OnKeyEvent("keyup",e));window.addEventListener("dblclick",e=>this._OnMouseEvent("dblclick",e,DISPATCH_RUNTIME_AND_SCRIPT));window.addEventListener("wheel",e=>this._OnMouseWheelEvent("wheel",e));if(typeof PointerEvent!=="undefined"){window.addEventListener("pointerdown",e=>{this._HandlePointerDownFocus(e);
this._OnPointerEvent("pointerdown",e)});if(this._iRuntime.UsesWorker()&&typeof window["onpointerrawupdate"]!=="undefined"&&self===self.top){this._pointerRawUpdateRateLimiter=new self.RateLimiter(()=>this._DoSendPointerRawUpdate(),5);this._pointerRawUpdateRateLimiter.SetCanRunImmediate(true);window.addEventListener("pointerrawupdate",e=>this._OnPointerRawUpdate(e))}else window.addEventListener("pointermove",e=>this._OnPointerEvent("pointermove",e));window.addEventListener("pointerup",e=>this._OnPointerEvent("pointerup",
e));window.addEventListener("pointercancel",e=>this._OnPointerEvent("pointercancel",e))}else{window.addEventListener("mousedown",e=>{this._HandlePointerDownFocus(e);this._OnMouseEventAsPointer("pointerdown",e)});window.addEventListener("mousemove",e=>this._OnMouseEventAsPointer("pointermove",e));window.addEventListener("mouseup",e=>this._OnMouseEventAsPointer("pointerup",e));window.addEventListener("touchstart",e=>{this._HandlePointerDownFocus(e);this._OnTouchEvent("pointerdown",e)});window.addEventListener("touchmove",
e=>this._OnTouchEvent("pointermove",e));window.addEventListener("touchend",e=>this._OnTouchEvent("pointerup",e));window.addEventListener("touchcancel",e=>this._OnTouchEvent("pointercancel",e))}const playFunc=()=>this._PlayPendingMedia();window.addEventListener("pointerup",playFunc,true);window.addEventListener("touchend",playFunc,true);window.addEventListener("click",playFunc,true);window.addEventListener("keydown",playFunc,true);window.addEventListener("gamepadconnected",playFunc,true)}_PostRuntimeEvent(name,
...
"clientX":e.clientX,"clientY":e.clientY,"pageX":e.pageX,"pageY":e.pageY,"timeStamp":e.timeStamp},opts)}_OnMouseEventAsPointer(name,e){if(IsCompatibilityMouseEvent(e))return;const pointerId=1;const lastButtons=this._mousePointerLastButtons;if(name==="pointerdown"&&lastButtons!==0)name="pointermove";else if(name==="pointerup"&&e.buttons!==0)name="pointermove";this._PostToRuntimeMaybeSync(name,{"pointerId":pointerId,"pointerType":"mouse","button":e.button,"buttons":e.buttons,"lastButtons":lastButtons,
"clientX":e.clientX,"clientY":e.clientY,"pageX":e.pageX,"pageY":e.pageY,"width":0,"height":0,"pressure":0,"tangentialPressure":0,"tiltX":0,"tiltY":0,"twist":0,"timeStamp":e.timeStamp},DISPATCH_RUNTIME_AND_SCRIPT);this._mousePointerLastButtons=e.buttons;this._OnMouseEvent(e.type,e,DISPATCH_SCRIPT_ONLY)}_OnPointerEvent(name,e){if(this._pointerRawUpdateRateLimiter&&name!=="pointermove")this._pointerRawUpdateRateLimiter.Reset();let lastButtons=0;if(e.pointerType==="mouse")lastButtons=this._mousePointerLastButtons;
this._PostToRuntimeMaybeSync(name,{"pointerId":e.pointerId,"pointerType":e.pointerType,"button":e.button,"buttons":e.buttons,"lastButtons":lastButtons,"clientX":e.clientX,"clientY":e.clientY,"pageX":e.pageX,"pageY":e.pageY,"width":e.width||0,"height":e.height||0,"pressure":e.pressure||0,"tangentialPressure":e["tangentialPressure"]||0,"tiltX":e.tiltX||0,"tiltY":e.tiltY||0,"twist":e["twist"]||0,"timeStamp":e.timeStamp},DISPATCH_RUNTIME_AND_SCRIPT);if(e.pointerType==="mouse"){let mouseEventName="mousemove";
if(name==="pointerdown")mouseEventName="mousedown";else if(name==="pointerup")mouseEventName="mouseup";this._OnMouseEvent(mouseEventName,e,DISPATCH_SCRIPT_ONLY);this._mousePointerLastButtons=e.buttons}}_OnPointerRawUpdate(e){this._lastPointerRawUpdateEvent=e;this._pointerRawUpdateRateLimiter.Call()}_DoSendPointerRawUpdate(){this._OnPointerEvent("pointermove",this._lastPointerRawUpdateEvent);this._lastPointerRawUpdateEvent=null}_OnTouchEvent(fireName,e){for(let i=0,len=e.changedTouches.length;i<len;++i){const t=
e.changedTouches[i];this._PostToRuntimeMaybeSync(fireName,{"pointerId":t.identifier,"pointerType":"touch","button":0,"buttons":0,"lastButtons":0,"clientX":t.clientX,"clientY":t.clientY,"pageX":t.pageX,"pageY":t.pageY,"width":(t["radiusX"]||t["webkitRadiusX"]||0)*2,"height":(t["radiusY"]||t["webkitRadiusY"]||0)*2,"pressure":t["force"]||t["webkitForce"]||0,"tangentialPressure":0,"tiltX":0,"tiltY":0,"twist":t["rotationAngle"]||0,"timeStamp":e.timeStamp},DISPATCH_RUNTIME_AND_SCRIPT)}}_HandlePointerDownFocus(e){if(window!==
window.top)window.focus();if(this._IsElementCanvasOrDocument(e.target)&&document.activeElement&&!this._IsElementCanvasOrDocument(document.activeElement))document.activeElement.blur()}_IsElementCanvasOrDocument(elem){return!elem||elem===document||elem===window||elem===document.body||elem.tagName.toLowerCase()==="canvas"}_AttachDeviceOrientationEvent(){if(this._attachedDeviceOrientationEvent)return;this._attachedDeviceOrientationEvent=true;window.addEventListener("deviceorientation",e=>this._OnDeviceOrientation(e));
window.addEventListener("deviceorientationabsolute",e=>this._OnDeviceOrientationAbsolute(e))}_AttachDeviceMotionEvent(){if(this._attachedDeviceMotionEvent)return;this._attachedDeviceMotionEvent=true;window.addEventListener("devicemotion",e=>this._OnDeviceMotion(e))}_OnDeviceOrientation(e){this.PostToRuntime("deviceorientation",{"absolute":!!e["absolute"],"alpha":e["alpha"]||0,"beta":e["beta"]||0,"gamma":e["gamma"]||0,"timeStamp":e.timeStamp,"webkitCompassHeading":e["webkitCompassHeading"],"webkitCompassAccuracy":e["webkitCompassAccuracy"]},
...
msg)}};RuntimeInterface.AddDOMHandlerClass(HANDLER_CLASS)};
...

{
	...
	const DOM_COMPONENT_ID = "cv-playfab-api";

	const HANDLER_CLASS = class PlayFab_API_Handler extends self.DOMElementHandler
	{
		constructor(iRuntime)
		{
			...
			this.AddRuntimeMessageHandlers([
				["call-api-method", e => this._CallAPIMethod(e)],
				["set-title-id", e => this._SetTitleID(e)],
				["forget-all-credentials", e => this._ForgetAllCredentials(e)]
			]);
		...
		Initialize()
		{
			...
			this.CallAPIMethod = (callback, error) => {
			
				const k = "call-api-method";
				self.PostToRuntime(k, [callback, error]);
			}
		}
		...
		_CallAPIMethod (e) {
			...
			const method = e[0];
			const data = e[1];
			const customData = e[2];

			window["PlayFab"]["ClientApi"][method](data, this.CallAPIMethod, customData);
		}
		_SetTitleID (e) {
			
			const titleID = e[0];

			window["PlayFab"]["settings"]["titleId"] = titleID;
		}
		_ForgetAllCredentials () {
			
			window["PlayFab"]["ClientApi"]["ForgetAllCredentials"]();
		}
...

{
	...
	const DOM_COMPONENT_ID = "cv-playfab-leaderboards";

	const HANDLER_CLASS = class PlayFab_Leaderboards_Handler extends self.DOMElementHandler
	{
		constructor(iRuntime)
		{
			super(iRuntime, DOM_COMPONENT_ID);
			...
			this.AddRuntimeMessageHandlers([
				["get-friend-leaderboard", e => this._GetFriendLeaderboard(e)],
				["get-friend-leaderboard-around-player", e => this._GetFriendLeaderboardAroundPlayer(e)],
				["get-leaderboard", e => this._GetLeaderboard(e)],
				["get-leaderboard-around-player", e => this._GetLeaderboardAroundPlayer(e)],
				["get-player-statistics", e => this._GetPlayerStatistics(e)],
				["get-player-statistic-versions", e => this._GetPlayerStatisticVersions(e)],
				["update-player-statistics", e => this._UpdatePlayerStatistics(e)]
			]);
		...
		Initialize()
		{
			...
			this.GetFriendLeaderboard = (callback, error) => {
			
				const k = "get-friend-leaderboard";
				self.PostToRuntime(k, [callback, error]);
			}
			this.GetFriendLeaderboardAroundPlayer = (callback, error) => {
			
				const k = "get-friend-leaderboard-around-player";
				self.PostToRuntime(k, [callback, error]);
			}
			this.GetLeaderboard = (callback, error) => {
			
				const k = "get-leaderboard";
				self.PostToRuntime(k, [callback, error]);
			}
			this.GetLeaderboardAroundPlayer = (callback, error) => {
			
				const k = "get-leaderboard-around-player";
				self.PostToRuntime(k, [callback, error]);
			}
			this.GetPlayerStatistics = (callback, error) => {
			
				const k = "get-player-statistics";
				self.PostToRuntime(k, [callback, error]);
			}
			this.GetPlayerStatisticVersions = (callback, error) => {
			
				const k = "get-player-statistic-versions";
				self.PostToRuntime(k, [callback, error]);
			}
			this.UpdatePlayerStatistics = (callback, error) => {
			
				const k = "update-player-statistics";
				self.PostToRuntime(k, [callback, error]);
			}
		}
		...
		//////////////////////////////////////
        ///// DOM Methods
		_GetFriendLeaderboard (e) {
		
			const data = e[0];
			const customData = e[1];

			window["PlayFab"]["ClientApi"]["GetFriendLeaderboard"](data, this.GetFriendLeaderboard, customData);
		}
		
		_GetFriendLeaderboardAroundPlayer (e) {
			
			const data = e[0];
			const customData = e[1];

			window["PlayFab"]["ClientApi"]["GetFriendLeaderboardAroundPlayer"](data, this.GetFriendLeaderboardAroundPlayer, customData);	
		}
		...
		_GetLeaderboard (e) {
			...
			window["PlayFab"]["ClientApi"]["GetLeaderboard"](data, this.GetLeaderboard, customData);
		}
		
		_GetLeaderboardAroundPlayer (e) {

			const data = e[0];
			const customData = e[1];
	
			window["PlayFab"]["ClientApi"]["GetLeaderboardAroundPlayer"](data, this.GetLeaderboardAroundPlayer, customData);	
		}
		
		_GetPlayerStatistics (e) {
			
			const data = e[0];
			const customData = e[1];

			window["PlayFab"]["ClientApi"]["GetPlayerStatistics"](data, this.GetPlayerStatistics, customData);
		}
		
		_GetPlayerStatisticVersions (e) {
			
			const data = e[0];

			window["PlayFab"]["ClientApi"]["GetPlayerStatisticVersions"](data, this.GetPlayerStatisticVersions);
		}

		_UpdatePlayerStatistics (e) {
			
			const data = e[0];
			const customData = e[1];

			window["PlayFab"]["ClientApi"]["UpdatePlayerStatistics"](data, this.UpdatePlayerStatistics, customData);	
		}
...

{
	...
	const DOM_COMPONENT_ID = "cv-playfab-authentication";

	const HANDLER_CLASS = class PlayFab_Authentication_Handler extends self.DOMElementHandler
	{
		constructor(iRuntime)
		{
			super(iRuntime, DOM_COMPONENT_ID);
			...
			this.AddRuntimeMessageHandlers([
				["login-with-android", e => this._AndroidDeviceLogin(e)],
				["login-with-ios", e => this._IOSDeviceLogin(e)],
				["login-with-custom-id", e => this._CustomIDLogin(e)],
				["login-with-facebook", e => this._FacebookLogin(e)],
				["login-with-google", e => this._GoogleLogin(e)],
				["login-with-email", e => this._CompanyEmailLogin(e)],
				["login-with-playfab", e => this._CompanyUsernameLogin(e)],
				["register-with-playfab", e => this._CompanyRegister(e)],
				["set-entity-token", e => this.SetEntityToken(e)],
				["set-session-ticket", e => this.SetSessionTicket(e)]
			]);
		...
		Initialize()
		{
			...
			this.AndroidDeviceLogin = (callback, error) =>
			{
				const k = "login-with-android";
				self.PostToRuntime(k, [callback, error]);
			}
	
			this.IOSDeviceLogin = (callback, error) =>
			{
				const k = "login-with-ios";
				self.PostToRuntime(k, [callback, error]);
			}
	
			this.CustomIDLogin = (callback, error) =>
			{
				const k = "login-with-custom-id";
				self.PostToRuntime(k, [callback, error]);
			}
	
			this.FacebookLogin = (callback, error) =>
			{
				const k = "login-with-facebook";
				self.PostToRuntime(k, [callback, error]);
			}
	
			this.GoogleLogin = (callback, error) =>
			{
				const k = "login-with-google";
				self.PostToRuntime(k, [callback, error]);
			}
	
			this.CompanyEmailLogin = (callback, error) =>
			{
				const k = "login-with-email";
				self.PostToRuntime(k, [callback, error]);
			}
			
			this.CompanyUsernameLogin = (callback, error) =>
			{
				const k = "login-with-playfab";
				self.PostToRuntime(k, [callback, error]);
			}
	
			this.CompanyRegister = (callback, error) =>
			{
				const k = "register-with-playfab";
				self.PostToRuntime(k, [callback, error]);
			}
		}
        ...
        _AndroidDeviceLogin(e)
        {
			const data = e[0];
			window["PlayFab"]["ClientApi"]["LoginWithAndroidDeviceID"](data, this.AndroidDeviceLogin);
        }

        _IOSDeviceLogin(e)
        {
			const data = e[0];
			window["PlayFab"]["ClientApi"]["LoginWithIOSDeviceID"](data, this.IOSDeviceLogin);
        }

        _CustomIDLogin(e)
        {
			const data = e[0];
			window["PlayFab"]["ClientApi"]["LoginWithCustomID"](data, this.CustomIDLogin);
        }

        _FacebookLogin(e)
        {
			const data = e[0];
			window["PlayFab"]["ClientApi"]["LoginWithFacebook"](data, this.FacebookLogin);
        }

        _GoogleLogin(e)
        {
			const data = e[0];
			window["PlayFab"]["ClientApi"]["LoginWithGoogleAccount"](data, this.GoogleLogin);
        }

        _CompanyEmailLogin(e)
        {
			const data = e[0];
			window["PlayFab"]["ClientApi"]["LoginWithEmailAddress"](data, this.CompanyEmailLogin);
		}
		
		_CompanyUsernameLogin(e)
        {
			const data = e[0];
			window["PlayFab"]["ClientApi"]["LoginWithPlayFab"](data, this.CompanyUsernameLogin);
        }

        _CompanyRegister(e)
        {
			const data = e[0];
			window["PlayFab"]["ClientApi"]["RegisterPlayFabUser"](data, this.CompanyRegister);
		}
		
		SetEntityToken(e)
        {
			const token = e[0];
            window["PlayFab"]["_internalSettings"]["entityToken"] = token;
		}
		
        SetSessionTicket(e)
        {
			const ticket = e[0];
			window["PlayFab"]["_internalSettings"]["sessionTicket"] = ticket;

			const k = "set-session-ticket";
			this.PostToRuntime(k, [ticket]);
        }
...

{
	...
	const DOM_COMPONENT_ID = "cv-playfab-accountmanager";

	const HANDLER_CLASS = class PlayFab_AccountManager_Handler extends self.DOMElementHandler
	{
		constructor(iRuntime)
		{
			super(iRuntime, DOM_COMPONENT_ID);
			...
			this.AddRuntimeMessageHandlers([
				["send-account-recovery-email", e => this._SendAccountRecoveryEmail(e)],
				["add-username-password", e => this._AddUsernamePassword(e)],
				["add-or-update-contact-email", e => this._AddOrUpdateContactEmail(e)],
				["remove-contact-email", e => this._RemoveContactEmail(e)],
				["add-generic-id", e => this._AddGenericID(e)],
				["remove-generic-id", e => this._RemoveGenericID(e)],
				["update-avatar-url", e => this._UpdateAvatarURL(e)],
				["update-user-title-display-name", e => this._UpdateUserTitleDisplayName(e)],
				["report-player", e => this._ReportPlayer(e)]
			]);
		...
		Initialize()
		{
			...
			this.SendAccountRecoveryEmail = (callback, error) => {
			
				const k = "send-account-recovery-email";
				self.PostToRuntime(k, [callback, error]);
			}

			this.AddUsernamePassword = (callback, error) => {
			
				const k = "add-username-password";
				self.PostToRuntime(k, [callback, error]);
			}

			this.AddOrUpdateContactEmail = (callback, error) => {
			
				const k = "add-or-update-contact-email";
				self.PostToRuntime(k, [callback, error]);
			}

			this.RemoveContactEmail = (callback, error) => {
			
				const k = "remove-contact-email";
				self.PostToRuntime(k, [callback, error]);
			}

			this.AddGenericID = (callback, error) => {
			
				const k = "add-generic-id";
				self.PostToRuntime(k, [callback, error]);
			}

			this.RemoveGenericID = (callback, error) => {
			
				const k = "remove-generic-id";
				self.PostToRuntime(k, [callback, error]);
			}

			this.UpdateAvatarURL = (callback, error) => {
			
				const k = "update-avatar-url";
				self.PostToRuntime(k, [callback, error]);
			}

			this.UpdateUserTitleDisplayName = (callback, error) => {
			
				const k = "update-user-title-display-name";
				self.PostToRuntime(k, [callback, error]);
			}

			this.ReportPlayer = (callback, error) => {
			
				const k = "report-player";
				self.PostToRuntime(k, [callback, error]);
			}
		}
		...
		//////////////////////////////////////
        ///// DOM Methods
		_SendAccountRecoveryEmail (e) {
		
			const data = e[0];
			const customData = e[1];
			
			window["PlayFab"]["ClientApi"]["SendAccountRecoveryEmail"](data, this.SendAccountRecoveryEmail, customData);
		}
		
		_AddUsernamePassword (e){

			const data = e[0];
			const customData = e[1];	
			
			window["PlayFab"]["ClientApi"]["AddUsernamePassword"](data, this.AddUsernamePassword, customData);
		}
		
		_AddOrUpdateContactEmail (e) {
			
			const data = e[0];
			const customData = e[1];
			
			window["PlayFab"]["ClientApi"]["AddOrUpdateContactEmail"](data, this.AddOrUpdateContactEmail, customData);
		}
		
		_RemoveContactEmail (e) {
			
			const data = e[0];
			const customData = e[1];

			window["PlayFab"]["ClientApi"]["RemoveContactEmail"](data, this.RemoveContactEmail, customData);
		}
		...
		_AddGenericID (e) {
			...
			window["PlayFab"]["ClientApi"]["AddGenericID"](data, this.AddGenericID, customData);
		}
		...
		_RemoveGenericID (e) {
			...
			window["PlayFab"]["ClientApi"]["RemoveGenericID"](data, this.RemoveGenericID, customData);
		}
		...
		_UpdateAvatarURL (e) {
			...
			window["PlayFab"]["ClientApi"]["UpdateAvatarUrl"](data, this.UpdateAvatarURL, customData);
		}

		_UpdateUserTitleDisplayName (e) {
			
			const data = e[0];
			const customData = e[1];
			
			window["PlayFab"]["ClientApi"]["UpdateUserTitleDisplayName"](data, this.UpdateUserTitleDisplayName, customData);
		}
		...
		_ReportPlayer (e) {
			...
			window["PlayFab"]["ClientApi"]["ReportPlayer"](data, this.ReportPlayer, customData);
		}
